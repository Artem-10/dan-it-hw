...
def write_students_to_file(students):
    """Полностью перезаписывает CSV-файл списком студентов."""
...
def get_all_or_by_lastname():
    """Получить всех студентов или отфильтровать по фамилии."""
    students = read_students_from_file()
...
def update_student_put(student_id):
    """Полностью обновить информацию о студенте."""
    if not request.json:
        return jsonify({"error": "No data provided"}), 400
...
def update_student_patch(student_id):
    """Частично обновить информацию (только возраст)."""
    if not request.json:
        return jsonify({"error": "No data provided"}), 400
...
def delete_student(student_id):
    ...
    # Создаем новый список без удаленного студента
